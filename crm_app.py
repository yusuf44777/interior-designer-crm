...
menu = st.sidebar.selectbox(
    "Menü",
    ["🏠 Ana Sayfa", "👥 İç Mimarlar", "➕ İç Mimar Ekle", "✏️ İç Mimar Düzenle", "📅 Randevular", "📆 Randevu Ekle", "📊 İstatistikler", "📤 İçe/Dışa Aktar"]
)
...
# İç Mimar Düzenle Sayfası
elif menu == "✏️ İç Mimar Düzenle":
    st.header("✏️ İç Mimar Bilgilerini Düzenle")
    
    df = load_designers()
    
    if len(df) > 0:
        # İç mimar seçimi
        st.subheader("1️⃣ Düzenlemek İstediğiniz İç Mimarı Seçin")
        
        # Arama özelliği
        search_term = st.text_input("🔍 İç mimar ara (isim, şirket)", "")
        ...
        if len(filtered_df) > 0:
            ...
            for idx, row in filtered_df.iterrows():
                name = f"{row['first name']} {row['last name']} - {row['company name']}"
                designer_options.append(name)
                designer_indices.append(idx)
            
            selected_index = st.selectbox(
                "İç mimar seçin:",
                range(len(designer_options)),
                format_func=lambda i: designer_options[i]
            )
            
            # Seçilen iç mimarın bilgilerini al
            selected_designer_idx = designer_indices[selected_index]
            selected_designer = df.loc[selected_designer_idx]
            
            st.markdown("---")
            
            # Mevcut bilgileri göster
            st.subheader("2️⃣ Mevcut Bilgiler")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.info(f"**Ad:** {selected_designer['first name']}")
                st.info(f"**Soyad:** {selected_designer['last name']}")
                st.info(f"**Şirket:** {selected_designer['company name']}")
            
            with col2:
                st.info(f"**Email:** {selected_designer['email'] if selected_designer['email'] else 'Yok'}")
                st.info(f"**Telefon:** {selected_designer['phone'] if selected_designer['phone'] else 'Yok'}")
                st.info(f"**Adres:** {selected_designer['address'] if selected_designer['address'] else 'Yok'}")
            
            if selected_designer['linkedin adress']:
                st.info(f"**LinkedIn:** {selected_designer['linkedin adress']}")
            
            st.markdown("---")
            
            # Düzenleme formu
            st.subheader("3️⃣ Yeni Bilgileri Girin")
            
            with st.form("edit_designer_form", clear_on_submit=False):
                col1, col2 = st.columns(2)
                
                with col1:
                    new_first_name = st.text_input(
                        "Ad *", 
                        value=selected_designer['first name']
                    )
                    new_last_name = st.text_input(
                        "Soyad *", 
                        value=selected_designer['last name']
                    )
                    new_email = st.text_input(
                        "Email", 
                        value=selected_designer['email'] if pd.notna(selected_designer['email']) else ""
                    )
                    new_phone = st.text_input(
                        "Telefon", 
                        value=selected_designer['phone'] if pd.notna(selected_designer['phone']) else ""
                    )
                
                with col2:
                    new_company_name = st.text_input(
                        "Şirket Adı *", 
                        value=selected_designer['company name']
                    )
                    new_address = st.text_input(
                        "Adres", 
                        value=selected_designer['address'] if pd.notna(selected_designer['address']) else ""
                    )
                    new_linkedin = st.text_input(
                        "LinkedIn Adresi", 
                        value=selected_designer['linkedin adress'] if pd.notna(selected_designer['linkedin adress']) else ""
                    )
                ...
                if submitted:
                    if not new_first_name or not new_last_name or not new_company_name:
                        st.error("❌ Lütfen zorunlu alanları doldurun!")
                    else:
                        # Verileri güncelle
                        df.at[selected_designer_idx, 'first name'] = new_first_name
                        df.at[selected_designer_idx, 'last name'] = new_last_name
                        df.at[selected_designer_idx, 'company name'] = new_company_name
                        df.at[selected_designer_idx, 'email'] = new_email
                        df.at[selected_designer_idx, 'phone'] = new_phone
                        df.at[selected_designer_idx, 'address'] = new_address
                        df.at[selected_designer_idx, 'linkedin adress'] = new_linkedin
                        
                        save_designers(df)
                        
                        st.success(f"✅ {new_first_name} {new_last_name} başarıyla güncellendi!")
                        st.balloons()
                        st.rerun()
                
                if cancel:
                    st.info("İptal edildi.")
                    st.rerun()
        ...
        st.warning("Henüz düzenlenecek iç mimar bulunmuyor. Lütfen önce iç mimar ekleyin.")
